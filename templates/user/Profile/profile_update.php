<script type='text/javascript' >
	...
	function formsubmit()	
	{
		...
		if(flag == 1)
		{
			$.ajax({
				type: 'POST',
				url : 'Profile/profile_update_back.php',
				data: {qualification:id,institution:institution.value,university:university.value,location:location.value,join_year:join_year.value,pass_year:pass_year.value, percentage:percentage.value, class1:class1.value},
			}).done(function(data){
				if(data == ''){
...
<p class='div_award_field_text' style='color:red; display: block;'><b>Enter your Educational Qualification</b></p>
        ...
        <span class= "div_book_field_text">Join_Year</span>
		  <span id="star">*</span>
        <input class="book_form_input" type="month" name="join_year" id="join_year" maxlength=4>
        <br/>
        <br/>
