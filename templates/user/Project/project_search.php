<?php	
		...
		$query="select project_id FROM project where id = '$id'  and  citation_index like '$citation_index' and principal_investigator like '$principal_investigator' and project_title like '$project_title'  and  project_status like '$project_status' and project_cost like '$project_cost' and abstract like '$abstract' and url like '$url' and funding_agency like '$funding_agency' and start_date >= '$start_date' and  start_date <= '$close_date' and close_date >= '$start_date' and  close_date <= '$close_date'";
		...
		if($chapters[0] != 0)
		{
			...
			{	
				...
				for($j = 0; $j < $num; $j++)
				{	
					$query = "select * from project_co_investigator where id = '$id' and project_id = '$project_id_array[$j]' and co_investigaor_id = '$chapters[$i]'";
					$result = mysql_query($query);
					if(mysql_num_rows($result) != 0)
					{	
						$project_id_array[$p] = $project_id_array[$j];
		...
		for ($i = 0; $i < $num; $i++){
			...
			for($j = 0; $j < $n; $j++)
			{
				$r = mysql_fetch_array($res);
				if($j == $n - 1)
					$chapters = $chapters.$r['co_investigaor_id'];
				else
					$chapters = $chapters.$r['co_investigaor_id'].', ';
			}
