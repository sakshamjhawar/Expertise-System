...
<p class='div_award_field_text'></p>
		<p class='div_award_field_text' style='color:red; display: block;'><b>Enter the Book details to filter search results</b></p>
		<span class= "div_book_field_text">Bood ID</span>
        <span id="star">*</span>
...
<script type='text/javascript' >
		...
		$.ajax({
		  ...
		  data: { book_id : book_id, book_or_chapter:book_or_chapter , book_title: book_title,role: role, book_edition: book_edition, authors: authors_jsonString, chapters: chapters_jsonString, publisher_name:publisher_name, isbn:isbn, from_date : from_date, to_date : to_date}
			...
			//alert(d);
				data = d.split(";");
				if(data.length > 1){
				document.getElementById("book_delete").innerHTML = '<table class = "div_table_award_overview" cellpadding = "15" style = "word-wrap: break-word;">\
				<thead> \
					<tr> \
	...
	$.ajax({
		  type: "POST",
		  url: "Book/book_generate_report_back.php",
